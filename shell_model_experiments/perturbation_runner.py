...
from lyaponov.lyaponov_exp_estimator import find_eigenvector_for_perturbation,\
...
def perturbation_runner(u_old, perturb_positions, du_array, data_out, args,
    ...
    run_model(u_old, du_array, data_out, args['Nt'] + args['endpoint']*1, args['ny'], args['forcing'])
...
def main(args=None):
    ...
    data_out = np.zeros((int(args['Nt']*sample_rate) + args['endpoint']*1, n_k_vec + 1),
        dtype=np.complex128)
