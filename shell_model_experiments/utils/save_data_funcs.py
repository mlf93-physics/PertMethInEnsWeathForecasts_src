...
def save_data(data_out, subfolder="", prefix="", perturb_position=None, args=None):
    ...
    if len(subfolder) == 0:
        expected_name = (
            f"ny{temp_ny}_t{temp_time_to_run}" + f"_n_f{n_forcing}_f{temp_forcing}"
        )
    ...
    if args["ref_run"]:
        ...
        ref_data_info_name = (
            f"data/{subfolder}/{subsubfolder}/ref_data_info_ny"
    ...
    header = (
        f"f={args['forcing']}, n_f={n_forcing}, n_ny={args['ny_n']}, ny={args['ny']}, "
    ...
    # Save data
    np.savetxt(
        f"data/{subfolder}/{subsubfolder}/{prefix}udata_ny{temp_ny}_t{temp_time_to_run}"
