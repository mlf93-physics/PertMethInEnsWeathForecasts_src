...
def plot_pert_vectors(args, axes: plt.Axes = None):
    ...
    for mode in pert_modes:
        ...
        perturbations, _, _ = r_utils.prepare_perturbations(
            args, raw_perturbations=True
        )

        axes.scatter(perturbations[0, :], perturbations[1, :], label=mode)

    title = g_plt_utils.generate_title(args, title_header="Perturbation Vectors")
